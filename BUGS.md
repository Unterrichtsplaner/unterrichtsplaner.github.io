...
- [ ] **P10** 🟢 „Zuletzt synchronisiert“ zeigt die Versionskennung der Cloud (Uhr des Geräts, das zuletzt hochgeladen hat), nicht den letzten Abgleich: gerade abgeglichen, Anzeige „vor 10 Tagen“ (`updateSyncUI` ~Z. 5522). *Test.* → Eigenes Feld `lastSyncCheck` (per `persistDB`) oder Beschriftung „Cloud-Stand vom …“.
- [ ] **P11** 🟢 Unlesbare Daten: Jeder Start legt eine weitere Rettungskopie `lehrerapp_v3_defekt_<Zeit>` an (`rescueUnreadableDB` ~Z. 112) → Speicher läuft voll, `persistDB` gesperrt; die Meldung bietet keinen „Herunterladen“-Knopf. *Test.* → Keine Kopie, wenn eine inhaltsgleiche existiert; Knopf im Hinweis.
- [ ] **P12** 🟢 **`npm test` ist rot** (Exit-Code 1, alle 561 Tests grün, aber „1 unhandled error“): `tests/block-o.test.js` Z. 43 feuert `click` auf das Overlay, dessen Inline-`onclick="closeModalOnOverlay(…)"` in jsdom ins Leere läuft (Regel 18). *Test.* → Im Test nur `app('closeModalOnOverlay')(…)` aufrufen bzw. das Inline-Attribut vorher entfernen.
